...
inline void DDS_writebits(unsigned int value,unsigned int bits)
   ...
   if (DDS_bufsize+bits<32)
      ...
      if (DDS_cachepos+4>DDS_cachesize)
      {
          ...
          {
              if ((DDS_cache = (unsigned char*)realloc(
                     DDS_cache, DDS_cachesize + DDS_BLOCKSIZE)) == NULL)
                  MEMERROR();
              DDS_cachesize += DDS_BLOCKSIZE;
          }
      }
   ...
   while ((cnt1=DDS_readbits(DDS_RL))!=0)
      ...
      for (cnt2=0; cnt2<cnt1; cnt2++)
         ...
         if ((cnt&(DDS_BLOCKSIZE-1))==0)
         {
             ...
             {
                 if ((ptr1 = (unsigned char*)realloc(
                        ptr1, cnt + DDS_BLOCKSIZE)) == NULL)
                     MEMERROR();
                 ptr2 = &ptr1[cnt];
             }
         }
...
unsigned char *readRAWfiled(FILE *file,unsigned int *bytes)
   ...
   do
      ...
      blkcnt=(unsigned int)(fread(&data[cnt],1,DDS_BLOCKSIZE,file));
      cnt+=blkcnt;
      }
...
void writePVMvolume(const char *filename,unsigned char *volume,
   ...
   if (description==NULL && courtesy==NULL && parameter==NULL && comment==NULL)
      ...
      writeDDSfile(filename,data,(unsigned int)(strlen(str))+width*height*depth*components,components,width);
      }
   else
      {
      if (description!=NULL) len1=(unsigned int)(strlen((char *)description))+1;
      if (courtesy!=NULL) len2=(unsigned int)(strlen((char *)courtesy))+1;
      if (parameter!=NULL) len3=(unsigned int)(strlen((char *)parameter))+1;
      if (comment!=NULL) len4=(unsigned int)(strlen((char *)comment))+1;
      ...
      writeDDSfile(filename,data,(unsigned int)(strlen(str))+width*height*depth*components+len1+len2+len3+len4,components,width);
      }
   }
...
unsigned char *readPVMvolume(const char *filename,
   ...
   ptr=(unsigned char *)strchr((char *)ptr,'\n')+1;
   if (version==3) len1=(unsigned int)(strlen((char *)(ptr+(*width)*(*height)*(*depth)*numc)))+1;
   if (version==3) len2=(unsigned int)(strlen((char *)(ptr+(*width)*(*height)*(*depth)*numc+len1)))+1;
   if (version==3) len3=(unsigned int)(strlen((char *)(ptr+(*width)*(*height)*(*depth)*numc+len1+len2)))+1;
   if (version==3) len4=(unsigned int)(strlen((char *)(ptr+(*width)*(*height)*(*depth)*numc+len1+len2+len3)))+1;
   ...
   if (parameter!=NULL)
   {
       if (len3 > 1)
           *parameter =
             volume + (*width) * (*height) * (*depth) * numc + len1 + len2;
       else
           *parameter = NULL;
   }
   ...
   {
       if (len4 > 1)
           *comment = volume + (*width) * (*height) * (*depth) * numc + len1 +
                      len2 + len3;
       else
           *comment = NULL;
   }
